...
impl LifecyclePhase for ScanPhase {
    fn execute(&self, context: &mut StowawayContext) -> Result<()> {
        info!(source_dir = ?context.source_dir, "Scanning files");

        let include_patterns: Vec<Pattern> = context
            .config
            .interpolation
            .include_patterns
            .iter()
        ...
        let exclude_patterns: Vec<Pattern> = context
            .config
            .interpolation
            .exclude_patterns
            .iter()
        ...
        for entry in WalkDir::new(&context.source_dir) {
            let entry = entry?;
            debug!("Scanning file: {}", entry.path().to_string_lossy());
            if entry.file_type().is_file() {
                let relative_path = entry
                    .path()
                    .strip_prefix(&context.source_dir)
        ...
        info!(file_count = context.files.len(), "Found files to process");
        Ok(())
    }
}
...
mod tests {
    ...
    fn test_collects_include_files() {
        ...
        let txt_file = context
            .files
            .iter()
            .find(|f| f.relative_path.to_string_lossy() == "config.txt")
            .unwrap();
        ...
        let md_file = context
            .files
            .iter()
            .find(|f| f.relative_path.to_string_lossy() == "readme.md")
            .unwrap();
        ...
        let sh_file = context
            .files
            .iter()
            .find(|f| f.relative_path.to_string_lossy() == "script.sh")
            .unwrap();
    ...
    fn test_recurses_directories() {
        ...
        for file_entry in &context.files {
            ...
            assert_eq!(
                file_entry.target_path,
                context.target_dir.join(&file_entry.relative_path)
            );
        }
    }
    ...
    fn test_multiple_include_patterns() {
        ...
        let mut context = create_test_context(
            &temp_dir,
            vec!["*.txt".to_string(), "*.conf".to_string()],
            vec![],
        );
        ...
        let conf_file = context
            .files
            .iter()
            .find(|f| f.relative_path.to_string_lossy() == "app.conf")
            .unwrap();
    ...
    fn test_complex_glob_patterns() {
        ...
        let interpolated_files: Vec<String> = context
            .files
            .iter()
        ...
        let all_files: Vec<String> = context
            .files
            .iter()
            .map(|f| f.relative_path.to_string_lossy().to_string())
            .collect();
    ...
    fn test_directories_ignored() {
        ...
        create_test_files(&context.source_dir, vec![("file.txt", "content")]);
    ...
    fn test_invalid_glob_pattern() {
        ...
        create_test_files(&context.source_dir, vec![("test.txt", "content")]);
