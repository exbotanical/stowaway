...
bool
file_is_pointer (const char* filepath) {
...
bool
file_is_symlink (const char* filepath) {
  struct stat st;
...
bool
file_is_directory (const char* filepath) {
  struct stat st;
  return (stat(filepath, &st) == 0) && S_ISDIR(st.st_mode);
}

bool
file_is_symlinked_to (const char* filepath, const char* link) {
